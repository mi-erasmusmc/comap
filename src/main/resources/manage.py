...
def add_user_to_project(username, project):
    ...
    cur.execute("INSERT INTO users_projects (user_id, project_id) VALUES (%s, %s)",
                [user_id, project_id])
    print("Adding user %d to project %d with ID %d", user_id, project_id, db.insert_id())
    db.commit()
...
def show():
    ...
    for (project_name, project_id), ups in groupby(ups, lambda up: (up['name'], up['id'])):
        prefix = " " * (longest - len(project_name))
        print("\n## %d. %s" % (project_id, project_name))
        print("\nUsers: %s" % ", ".join(up['username'] for up in ups))
        print("\nCase definitions\n")
        cur.execute('SELECT * FROM case_definitions WHERE project_id = %s', [project_id])
