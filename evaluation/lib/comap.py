...
def mapping_to_max_recall_cuis_aux(coding_systems, cosynonyms, mapping, min_codes=None):
    ...
    ...    'E': {'V1': {1}, 'V2': {5}},
    ... }
    >>> mapping_to_max_recall_cuis_aux(coding_systems, cosynonyms, mapping) ==\
         { 'A', 'B', 'C' }
    True
    >>> mapping_to_max_recall_cuis_aux(coding_systems, cosynonyms, mapping, min_codes=2) ==\
         {'A'}
    True
    """

    assert isinstance(coding_systems, dict), type(coding_systems)
    ...
    for database_id, reference in mapping.items():
        coding_system = coding_systems[database_id]
                    ...
                    if code in cosynonyms[cui][coding_system]:
                        ...
                        for database_id0, reference0 in mapping.items():
                            if reference0 is not None:
                                coding_system0 = coding_systems[database_id0]
...
def mapping_to_max_recall_cuis(databases, mapping, min_codes=None):
    ...
    """
    assert type(databases) == Databases, type(databases)
    ...
    for database in databases.databases():
        ...
        if codes is None:
            continue
        cui_codes = cosynonym_codes(codes,
    ...
    return mapping_to_max_recall_cuis_aux(databases.coding_systems_data(), all_cosynonyms,
                                          mapping.to_data(), min_codes=min_codes)
