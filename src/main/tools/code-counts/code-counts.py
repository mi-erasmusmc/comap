...
Code counts*: A CSV file with the code counts generated in event fingerprinting
(with columns `Database`, `EventType`, `EventCode`, `Count`, `FirstCount`).
...
def run(vocabularies, code_counts_csv, code_names_csv, comap_xls_files, output_xls):
    ...
    code_counts = get_code_counts(code_counts_csv)            # Event × Code × Count × FirstCount
    code_counts['Extracted code'] = code_counts['Code']
