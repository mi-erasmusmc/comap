  ...
  <body ng-controller="CodeMapperCtrl">
    ...
    <div class="row">
      <div class="col-md-10">
        ...
        <div>Case definition: {{caseDefinitionName}}</div>
      </div>
      <div class="col-md-2">
    ...
    <tabset>
      ...
      <tab id="concepts-tab" heading="4. Concepts">
        <div id="concept-buttons">
          <div>
            Case definition with {{concepts.length}} concepts
          </div>
    ...
    <script type="text/ng-template" id="CreateOpenCaseDefinition.html">
       <div class="modal-header">
         <h3 class="modal-title">Create/open case definition</h3>
       </div>
       <div class="modal-body">
         <form id="selectCreateOpenCaseDefinition">
           <div>
             <label>
               <input type=radio name="selection" ng-model="selection" ng-value='' />
               Create <input type=text ng-model="createCaseDefinitionName" />
             </label>
           </div>
           <div ng-repeat="caseDefinition in caseDefinitions"> 
             <label>
               <input type=radio name='selection' ng-model="$parent.selection" ng-value="caseDefinition" required />
               {{caseDefinition}}
             </label>
           </div>
         </form>
       ...
       <div class="modal-footer">
         <button class="btn btn-primary" ng-click="ok()" ng-disabled="!selection && !validCreation(createCaseDefinitionName)">
           <span ng-if="selection">Open</span>
           <span ng-if="!selection">Create</span>
         </button>
         <button class="btn btn-warning" ng-if="cancellable" ng-click="cancel()">Cancel</button>
       </div>
    </script>
  </body>
</html>
