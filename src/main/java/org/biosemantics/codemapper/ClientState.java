...
public class ClientState {
	...
	@JsonSubTypes({ @Type(value = OriginBroader.class, name = "broader"),
	...
	@JsonSubTypes({ @Type(value = HistoryEntryAdd.class, name = "Add"),
	...
	public static class HistoryEntryExpandMoreGeneral extends HistoryEntry<ShortConcept[], ShortConcept[]> {}
	public static class HistoryEntryExpandMoreSpecific extends HistoryEntry<ShortConcept[], ShortConcept[]> {}
	public static class HistoryEntryChangeCodingSystems extends HistoryEntry<String, String> {}

	/**
	 * Test the conversion of client JSON states to ClientState$State on all
		...
		for (String project : persistencyApi.getProjects()) {
			for (String casedef : persistencyApi.getCaseDefinitionsNames(project)) {
				...
				try {
					...
					System.out.println("OK " + project + "/" + casedef + ": " + state.mapping.concepts.length);
